...
public class PrimitiveShuriken extends Armor {

    private Animation<TextureRegion> mAttackAnimation;

    public PrimitiveShuriken(final PlayScreen pPlayScreen, final float pX, final float pY, final boolean pDirection) {
        super(pPlayScreen, pX, pY, pDirection);
        setSize(ConstArmor.PRIMITIVE_SHURIKEN_SIZE - 0.2f, ConstArmor.PRIMITIVE_SHURIKEN_SIZE);
        setOriginCenter();
        for (int i = 0; i < 2; i++) {
            frames.add(new TextureRegion(DefenderOfNature.getArmorsAtlas().findRegion("armor_ninja3"), i * 89, 0, 89, 89));
    ...
    public int getArmorDamage() {
        return ConstArmor.PRIMITIVE_SHURIKEN_DAMAGE;
    }
