...
public class ModernShuriken extends Armor {

    private Animation<TextureRegion> mAttackAnimation;

    public ModernShuriken(final PlayScreen pPlayScreen, final float pX, final float pY, final boolean pDirection) {
        super(pPlayScreen, pX, pY, pDirection);
        setSize(ConstArmor.MODERN_SHURIKEN_SIZE, ConstArmor.MODERN_SHURIKEN_SIZE + 0.3f);
        setOriginCenter();
        for (int i = 0; i < 2; i++) {
            frames.add(new TextureRegion(DefenderOfNature.getArmorsAtlas().findRegion("armor_ninja1"), i * 96, 0, 96, 96));
            mAttackAnimation = new Animation<TextureRegion>(ConstArmor.SHURIKEN_SPEED - 0.01f, frames, Animation.PlayMode.LOOP);
        }
        frames.clear();
    ...
    public int getArmorDamage() {
        return ConstArmor.MODERN_SHURIKEN_DAMAGE;
    }
