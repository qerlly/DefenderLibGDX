...
public class Stork extends Armor implements SpawningSystem {

    private Sound sound;
    private Animation<TextureRegion> mAttackAnimation;
    private final Array<com.dmtrdev.monsters.sprites.armors.flyings.BirdArmor> mSpawns;
    private final BlockingDeque<SpawnDef> mEnemiesToSpawn;
    private int mSplats;
    private final PlayScreen mPlayScreen;

    public Stork(final PlayScreen pPlayScreen, final float pX, final float pY, final boolean pDirection) {
        super(pPlayScreen, pX, pY, pDirection);
        setSize(ConstArmor.STORK_WIDTH, ConstArmor.STORK_HEIGHT);
        ...
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                frames.add(new TextureRegion(DefenderOfNature.getArmorsAtlas().findRegion("stork_armor_fly"), j * 250, i * 120, 250, 120));
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 4; j++) {
                frames.add(new TextureRegion(DefenderOfNature.getArmorsAtlas().findRegion("stork_armor_die"), j * 250, i * 120, 250, 120));
    ...
    public int getArmorDamage() {
        return ConstArmor.STORK_DAMAGE;
    }
    ...
    public void update(final float delta) {
        ...
        if (setToDestroy && effects) {
            if (!destroy) {
                ...
                time = 0;
                destroy = true;
                setSize(ConstArmor.EFFECT_MEDIUM_SIZE, ConstArmor.EFFECT_MEDIUM_SIZE);
