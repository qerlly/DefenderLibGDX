...
public class BarrelShot extends Armor {
    ...
    public BarrelShot(final PlayScreen pPlayScreen, final float pX, final float pY, final boolean pDirection) {
        super(pPlayScreen, pX, pY, pDirection);
        setSize(4.3f, 5.8f);
        setRegion(DefenderOfNature.getArmorsAtlas().findRegion("swinging_spike_default"));
        setOriginCenter();
        mTakenDamage = 0;
        mHp = ConstArmor.BARREL_SHOT_HP;
        mFlag = false;
        for (int i = 0; i < 2; i++) {
            for (int j = 0; j < 7; j++) {
    ...
    public int getArmorDamage() {
        return ConstArmor.BARREL_SHOT_DAMAGE;
    }
    ...
    public void update(final float delta) {
        ...
        if (setToDestroy && effects) {
            if (!destroy) {
                ...
                setSize(ConstArmor.EFFECT_BIG_SIZE - 1.6f, ConstArmor.EFFECT_BIG_SIZE - 1);
                world.destroyBody(body);
                time = 0;
                destroy = true;
