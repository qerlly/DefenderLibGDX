...
public class PrimitiveAxe extends Armor {

    private Animation<TextureRegion> mAttackAnimation;

    public PrimitiveAxe(final PlayScreen pPlayScreen, final float pX, final float pY, final boolean pDirection) {
        super(pPlayScreen, pX, pY, pDirection);
        setSize(ConstArmor.AXE_SIZE, ConstArmor.AXE_SIZE - 0.2f);
        setOriginCenter();
        for (int i = 0; i < 8; i++) {
            frames.add(new TextureRegion(DefenderOfNature.getArmorsAtlas().findRegion("armor_axe2"), i * 177, 0, 177, 177));
    ...
    public int getArmorDamage() {
        return ConstArmor.PRIMITIVE_AXE_DAMAGE;
    }
