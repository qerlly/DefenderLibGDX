...
public class TreeBomb extends Armor {

    private Animation<TextureRegion> mIdleAnimation;

    public TreeBomb(final PlayScreen pPlayScreen, final float pX, final float pY, final boolean pDirection) {
        super(pPlayScreen, pX, pY, pDirection);
        setSize(ConstArmor.SMALL_BOMB_SIZE, ConstArmor.SMALL_BOMB_SIZE + 1.7f);
        setOriginCenter();
        for (int i = 0; i < 4; i++) {
            frames.add(new TextureRegion(DefenderOfNature.getArmorsAtlas().findRegion("bombik_default"), i * 135, 0, 135, 380));
    ...
    public int getArmorDamage() {
        return ConstArmor.TREEBOMB_DAMAGE;
    }
