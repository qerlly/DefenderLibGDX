...
public class StoneBomb extends Armor {

    private Animation<TextureRegion> mIdleAnimation;

    public StoneBomb(final PlayScreen pPlayScreen, final float pX, final float pY, final boolean pDirection) {
        super(pPlayScreen, pX, pY, pDirection);
        setSize(ConstArmor.SMALL_BOMB_SIZE + 0.1f, ConstArmor.SMALL_BOMB_SIZE + 1.8f);
        setOriginCenter();
        for (int i = 0; i < 4; i++) {
            frames.add(new TextureRegion(DefenderOfNature.getArmorsAtlas().findRegion("bombik_medium"), i * 135, 0, 135, 317));
    ...
    public int getArmorDamage() {
        return ConstArmor.STONEBOMB_DAMAGE;
    }
