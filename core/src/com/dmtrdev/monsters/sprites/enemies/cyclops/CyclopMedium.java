...
public class CyclopMedium extends Enemy {

    public CyclopMedium(final PlayScreen pScreen, final float pX, final float pY, final boolean pDirection) {
        ...
        if(pScreen.getType().contains(ConstGame.SURVIVE_MODE)){
            if(pScreen.getGameGui().getGenerates().getWaveIndex() % 5 == 0) {
                hp = (ConstEnemies.CYCLOP_MEDIUM_HP / 2) + ((pScreen.getGameGui().getGenerates().getWaveIndex() / 5) * 40);
            } else if(pScreen.getGameGui().getGenerates().getWaveIndex() > 4 ){
                hp = (ConstEnemies.CYCLOP_MEDIUM_HP / 2) + ((int)Math.floor(Math.abs(pScreen.getGameGui().getGenerates().getWaveIndex()/5)) * 40);
            } else {
                hp = ConstEnemies.CYCLOP_MEDIUM_HP / 2;
            }
        } else {
            hp = ConstEnemies.CYCLOP_MEDIUM_HP;
        }
        ...
        for (int j = 0; j < 2; j++) {
            for (int i = 0; i < 4; i++) {
                ...
                frames.add(new TextureRegion(DefenderOfNature.getCyclopsAtlas().findRegion("cyclop_medium_run"), i * 215, j * 161, 215, 161));
        ...
        for (int j = 0; j < 2; j++) {
            for (int i = 0; i < 4; i++) {
                ...
                frames.add(new TextureRegion(DefenderOfNature.getCyclopsAtlas().findRegion("cyclop_medium_attack"), i * 212, j * 207, 212, 207));
    ...
    public void update(final float delta) {
        ...
        switch (getState()) {
            ...
            case DISABLE:
                ...
                disableTime -= delta;
                if (direction) {
                    setPosition(body.getPosition().x - getWidth() / 2 + 0.4f, body.getPosition().y - 1.3f);
                } else {
                    setPosition(body.getPosition().x - getWidth() / 2 - 0.4f, body.getPosition().y - 1.3f);
                }
                break;
            case ATTACK:
                setSize(ConstEnemies.CYCLOP_SIZE, ConstEnemies.CYCLOP_SIZE + 2.2f);
                if (attackAnimation.isAnimationFinished(attackTime) && plantAttack) {
                    ...
                    tree.setHp(ConstEnemies.CYCLOP_MEDIUM_DAMAGE);
                ...
                if (direction) {
                    body.setLinearVelocity(0, ConstEnemies.VELOCITY_Y);
                    setPosition(body.getPosition().x - getWidth() / 2, body.getPosition().y - ConstEnemies.CYCLOP_BODY_DIFF_SIZE);
                } else {
                    body.setLinearVelocity(0, ConstEnemies.VELOCITY_Y);
                    setPosition(body.getPosition().x - getWidth() / 2, body.getPosition().y - ConstEnemies.CYCLOP_BODY_DIFF_SIZE);
                }
                ...
                break;
            default:
                setSize(ConstEnemies.CYCLOP_SIZE + 0.2f, ConstEnemies.CYCLOP_SIZE + 0.3f);
                if (direction) {
    ...
    public int getEnemyDamage() {
        return ConstEnemies.CYCLOP_MEDIUM_DAMAGE;
    }

    @Override
    public int getScore() {
        return ConstEnemies.CYCLOP_MEDIUM_SCORE;
    }
