...
public class CyclopDefault extends Enemy {

    public CyclopDefault(final PlayScreen pScreen, final float pX, final float pY, final boolean pDirection) {
        ...
        if(pScreen.getType().contains(ConstGame.SURVIVE_MODE)){
            if(pScreen.getGameGui().getGenerates().getWaveIndex() % 5 == 0) {
                hp = (ConstEnemies.CYCLOP_DEFAULT_HP / 2) + ((pScreen.getGameGui().getGenerates().getWaveIndex() / 5) * 40);
            } else if(pScreen.getGameGui().getGenerates().getWaveIndex() > 4 ){
                hp = (ConstEnemies.CYCLOP_DEFAULT_HP / 2) + ((int)Math.floor(Math.abs(pScreen.getGameGui().getGenerates().getWaveIndex()/5)) * 40);
            } else {
                hp = ConstEnemies.CYCLOP_DEFAULT_HP / 2;
            }
        } else {
            hp = ConstEnemies.CYCLOP_DEFAULT_HP;
        }

        for (int i = 0; i < 7; i++) {
            frames.add(new TextureRegion(DefenderOfNature.getCyclopsAtlas().findRegion("cyclop_default_run"), 0, i * 142, 208, 142));
        ...
        for (int j = 0; j < 2; j++) {
            for (int i = 0; i < 4; i++) {
                ...
                frames.add(new TextureRegion(DefenderOfNature.getCyclopsAtlas().findRegion("cyclop_default_attack"), i * 210, j * 192, 210, 192));
    ...
    public void update(final float delta) {
        ...
        switch (getState()) {
            ...
            case DISABLE:
                ...
                disableTime -= delta;
                if (direction) {
                    setPosition(body.getPosition().x - getWidth() / 2 + 0.4f, body.getPosition().y - ConstEnemies.CYCLOP_HEAD_SIZE);
                } else {
                    setPosition(body.getPosition().x - getWidth() / 2 - 0.4f, body.getPosition().y - ConstEnemies.CYCLOP_HEAD_SIZE);
                }                break;
            case ATTACK:
                setSize(ConstEnemies.CYCLOP_SIZE + 0.2f, ConstEnemies.CYCLOP_SIZE + 1.5f);
                if (attackAnimation.isAnimationFinished(attackTime) && plantAttack) {
                    ...
                    tree.setHp(ConstEnemies.CYCLOP_DEFAULT_DAMAGE);
                ...
                if (direction) {
                    body.setLinearVelocity(0, ConstEnemies.VELOCITY_Y);
                    setPosition(body.getPosition().x - getWidth() / 2 + 0.4f, body.getPosition().y  - 1.12f);
                } else {
                    body.setLinearVelocity(0, ConstEnemies.VELOCITY_Y);
                    setPosition(body.getPosition().x - getWidth() / 2 - 0.4f, body.getPosition().y  - 1.1f);
                }
                setRegion(getFrame());
                attackTime += delta;
                break;
            default:
                setSize(ConstEnemies.CYCLOP_SIZE, ConstEnemies.CYCLOP_SIZE - 0.6f);
                if (direction) {
    ...
    public int getEnemyDamage() {
        return ConstEnemies.CYCLOP_DEFAULT_DAMAGE;
    }

    @Override
    public int getScore() {
        return ConstEnemies.CYCLOP_DEFAULT_SCORE;
    }
