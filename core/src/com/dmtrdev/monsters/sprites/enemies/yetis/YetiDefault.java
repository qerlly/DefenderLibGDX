...
public class YetiDefault extends Enemy {

    private Animation<TextureRegion> jumpAnimation;
    private boolean jump;

    public YetiDefault(final PlayScreen pScreen, final float pX, final float pY, final boolean pDirection) {
        ...
        if(pScreen.getType().contains(ConstGame.SURVIVE_MODE)){
            if(pScreen.getGameGui().getGenerates().getWaveIndex() % 5 == 0) {
                hp = (ConstEnemies.YETI_DEFAULT_HP / 2) + ((pScreen.getGameGui().getGenerates().getWaveIndex() / 5) * 20);
            } else if(pScreen.getGameGui().getGenerates().getWaveIndex() > 4 ){
                hp = (ConstEnemies.YETI_DEFAULT_HP / 2) + ((int)Math.floor(Math.abs(pScreen.getGameGui().getGenerates().getWaveIndex()/5)) * 25);
            } else {
                hp = ConstEnemies.YETI_DEFAULT_HP / 2;
            }
        } else {
            hp = ConstEnemies.YETI_DEFAULT_HP;
        }

        for (int j = 0; j < 4; j++) {
            for (int i = 0; i < 2; i++) {
                frames.add(new TextureRegion(DefenderOfNature.getYetisAtlas().findRegion("yeti_default_run"), i * 330, i * 330, 330, 330));
        ...
        for (int j = 0; j < 2; j++) {
            for (int i = 0; i < 5; i++) {
                frames.add(new TextureRegion(DefenderOfNature.getYetisAtlas().findRegion("yeti_default_attack"), i * 330, j * 330, 330, 330));
        ...
        for (int i = 0; i < 5; i++) {
            frames.add(new TextureRegion(DefenderOfNature.getYetisAtlas().findRegion("yeti_default_jump"), i * 330, 0, 330, 330));
        ...
        switch (getState()) {
            ...
            case ATTACK:
                if (attackAnimation.isAnimationFinished(attackTime) && plantAttack) {
                    ...
                    tree.setHp(ConstEnemies.YETI_DEFAULT_DAMAGE);
                    attackTime = 0;
                }
                body.setLinearVelocity(0, ConstEnemies.VELOCITY_Y);
                setRegion(getFrame());
    ...
    public int getEnemyDamage() {
        return ConstEnemies.YETI_DEFAULT_DAMAGE;
    }

    @Override
    public int getScore() {
        return ConstEnemies.YETI_DEFAULT_SCORE;
    }
