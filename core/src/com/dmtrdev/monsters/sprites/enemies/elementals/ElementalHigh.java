...
public class ElementalHigh extends Enemy {

    public ElementalHigh(final PlayScreen pScreen, final float pX, final float pY, final boolean pDirection) {
        ...
        if(pScreen.getType().contains(ConstGame.SURVIVE_MODE)){
            if(pScreen.getGameGui().getGenerates().getWaveIndex() % 5 == 0) {
                hp = (ConstEnemies.ELEMENTAL_HIGH_HP / 2) + ((pScreen.getGameGui().getGenerates().getWaveIndex() / 5) * 15);
            } else if(pScreen.getGameGui().getGenerates().getWaveIndex() > 4 ){
                hp = (ConstEnemies.ELEMENTAL_HIGH_HP / 2) + ((int)Math.floor(Math.abs(pScreen.getGameGui().getGenerates().getWaveIndex()/5)) * 15);
            } else {
                hp = ConstEnemies.ELEMENTAL_HIGH_HP / 2;
            }
        } else {
            hp = ConstEnemies.ELEMENTAL_HIGH_HP;
        }

        for (int i = 0; i < 5; i++) {
            frames.add(new TextureRegion(DefenderOfNature.getElementalAtlas().findRegion("elemental_high_run"), i * 199, 0, 199, 243));
        ...
        for (int j = 0; j < 3; j++) {
            for (int i = 0; i < 2; i++) {
                ...
                frames.add(new TextureRegion(DefenderOfNature.getElementalAtlas().findRegion("elemental_high_attack"), i * 409, j * 260, 409, 260));
    ...
    public void damaged(final int pDamage, final String pData) {
        if (!pData.equals(Molotov.class.getSimpleName())) {
            hp -= pDamage;
        }
        if(hp <= 0){
            setToDestroy = true;
    ...
    public void update(final float delta) {
        ...
        switch (getState()) {
            case DIE:
                ...
                break;
            case ATTACK:
                setSize(ConstEnemies.ELEMENTAL_DEFAULT_SIZE + 2.2f, ConstEnemies.ELEMENTAL_DEFAULT_SIZE + 0.9f);
                if (attackAnimation.isAnimationFinished(attackTime) && plantAttack) {
                    ...
                    tree.setHp(ConstEnemies.ELEMENTAL_HIGH_DAMAGE);
                    attackTime = 0;
                }
                if (direction) {
                ...
                break;
            default:
                setSize(ConstEnemies.ELEMENTAL_DEFAULT_SIZE - 1, ConstEnemies.ELEMENTAL_DEFAULT_SIZE  + 0.7f);
                if (direction) {
    ...
    public int getEnemyDamage() {
        return ConstEnemies.ELEMENTAL_HIGH_DAMAGE;
    }

    @Override
    public int getScore() {
        return ConstEnemies.ELEMENTAL_HIGH_SCORE;
    }
    ...
    protected void defineEnemy() {
        ...
        fixtureDeff.filter.maskBits = ConstGame.ARMOR_ENEMY_BIT;
