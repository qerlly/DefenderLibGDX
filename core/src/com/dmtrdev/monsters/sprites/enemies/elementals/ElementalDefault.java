...
public class ElementalDefault extends Enemy {

    public ElementalDefault(final PlayScreen pScreen, final float pX, final float pY, final boolean pDirection) {
        ...
        if(pScreen.getType().contains(ConstGame.SURVIVE_MODE)){
            if(pScreen.getGameGui().getGenerates().getWaveIndex() % 5 == 0) {
                hp = (ConstEnemies.ELEMENTAL_DEFAULT_HP / 2) + ((pScreen.getGameGui().getGenerates().getWaveIndex() / 5) * 15);
            } else if(pScreen.getGameGui().getGenerates().getWaveIndex() > 4 ){
                hp = (ConstEnemies.ELEMENTAL_DEFAULT_HP / 2) + ((int)Math.floor(Math.abs(pScreen.getGameGui().getGenerates().getWaveIndex()/5)) * 15);
            } else {
                hp = ConstEnemies.ELEMENTAL_DEFAULT_HP / 2;
            }
        } else {
            hp = ConstEnemies.ELEMENTAL_DEFAULT_HP;
        }

        for (int i = 0; i < 5; i++) {
            frames.add(new TextureRegion(DefenderOfNature.getElementalAtlas().findRegion("elemental_default_run"), i * 197, 0, 197, 232));
        ...
        for (int j = 0; j < 3; j++) {
            for (int i = 0; i < 2; i++) {
                ...
                frames.add(new TextureRegion(DefenderOfNature.getElementalAtlas().findRegion("elemental_default_attack"), i * 411, j * 246, 411, 246));
    ...
    public void damaged(final int pDamage, final String pData) {
        hp -= pDamage;
        if (hp <= 0 || pData.equals(Grenade.class.getSimpleName()) || pData.equals(Bomb.class.getSimpleName()) || pData.equals(SpikeBomb.class.getSimpleName())) {
            setToDestroy = true;
        }
    }
    ...
    public void update(final float delta) {
        ...
        switch (getState()) {
            case DIE:
                ...
                break;
            case ATTACK:
                setSize(ConstEnemies.ELEMENTAL_DEFAULT_SIZE + 2.2f, ConstEnemies.ELEMENTAL_DEFAULT_SIZE + 0.2f);
                if (attackAnimation.isAnimationFinished(attackTime) && plantAttack) {
                    ...
                    tree.setHp(ConstEnemies.ELEMENTAL_DEFAULT_DAMAGE);
                    attackTime = 0;
                }
                if (direction) {
                ...
                break;
            default:
                setSize(ConstEnemies.ELEMENTAL_DEFAULT_SIZE - 1, ConstEnemies.ELEMENTAL_DEFAULT_SIZE);
                if (direction) {
    ...
    public int getEnemyDamage() {
        return ConstEnemies.ELEMENTAL_DEFAULT_DAMAGE;
    }

    @Override
    public int getScore() {
        return ConstEnemies.ELEMENTAL_DEFAULT_SCORE;
    }
