...
public class ZombieMedium extends Enemy {

    public ZombieMedium(final PlayScreen pScreen, final float pX, final float pY, final boolean pDirection) {
        ...
        if(pScreen.getType().contains(ConstGame.SURVIVE_MODE)){
            if(pScreen.getGameGui().getGenerates().getWaveIndex() % 5 == 0) {
                hp = (ConstEnemies.ZOMBIE_MEDIUM_HP / 2) + ((pScreen.getGameGui().getGenerates().getWaveIndex() / 5) * 10);
            } else if(pScreen.getGameGui().getGenerates().getWaveIndex() > 4 ){
                hp = (ConstEnemies.ZOMBIE_MEDIUM_HP / 2) + ((int)Math.floor(Math.abs(pScreen.getGameGui().getGenerates().getWaveIndex()/5)) * 10);
            } else {
                hp = ConstEnemies.ZOMBIE_MEDIUM_HP / 2;
            }
        } else {
            hp = ConstEnemies.ZOMBIE_MEDIUM_HP;
        }

        for (int i = 0; i < 5; i++) {
            frames.add(new TextureRegion(DefenderOfNature.getZombieAtlas().findRegion("zombie_medium_run"), i * 316, 0, 316, 367));
            runAnimation = new Animation<>(ConstEnemies.ZOMBIE_ANIM, frames, Animation.PlayMode.LOOP);
        }
        frames.clear();

        for (int i = 0; i < 5; i++) {
            frames.add(new TextureRegion(DefenderOfNature.getZombieAtlas().findRegion("zombie_medium_attack"), i * 316, 0, 316, 327));
        ...
        switch (getState()) {
            ...
            case ATTACK:
                ...
                if (attackAnimation.isAnimationFinished(attackTime) && plantAttack) {
                    ...
                    tree.setHp(ConstEnemies.ZOMBIE_MEDIUM_DAMAGE);
                    attackTime = 0;
                }
                body.setLinearVelocity(0, ConstEnemies.VELOCITY_Y);
                setRegion(getFrame());
    ...
    public int getEnemyDamage() {
        return ConstEnemies.ZOMBIE_MEDIUM_DAMAGE;
    }

    @Override
    public int getScore() {
        return ConstEnemies.ZOMBIE_MEDIUM_SCORE;
    }
