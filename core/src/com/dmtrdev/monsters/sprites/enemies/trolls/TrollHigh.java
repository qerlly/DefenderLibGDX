...
public class TrollHigh extends Enemy {

    public TrollHigh(final PlayScreen pScreen, final float pX, final float pY, final boolean pDirection) {
        ...
        if(pScreen.getType().contains(ConstGame.SURVIVE_MODE)){
            if(pScreen.getGameGui().getGenerates().getWaveIndex() % 5 == 0) {
                hp = (ConstEnemies.TROLL_HIGH_HP / 2) + ((pScreen.getGameGui().getGenerates().getWaveIndex() / 5) * 35);
            } else if(pScreen.getGameGui().getGenerates().getWaveIndex() > 4 ){
                hp = (ConstEnemies.TROLL_HIGH_HP / 2) + ((int)Math.floor(Math.abs(pScreen.getGameGui().getGenerates().getWaveIndex()/5)) * 35);
            } else {
                hp = ConstEnemies.TROLL_HIGH_HP / 2;
            }
        } else {
            hp = ConstEnemies.TROLL_HIGH_HP;
        }
        for (int i = 0; i < 6; i++) {
            frames.add(new TextureRegion(DefenderOfNature.getTrollsAtlas().findRegion("troll_high_run"), 0, i * 242, 345, 242));
        ...
        for (int j = 0; j < 4; j++) {
            for (int i = 0; i < 2; i++) {
                ...
                frames.add(new TextureRegion(DefenderOfNature.getTrollsAtlas().findRegion("troll_high_attack"), i * 395, j * 342, 395, 342));
    ...
    public void damaged(final int pDamage, final String pData) {
        hp -= pDamage;
        if (hp <= 0 || pData.equals(Bomb.class.getSimpleName()) || pData.equals(Grenade.class.getSimpleName()) || pData.equals(SpikeBomb.class.getSimpleName())) {
            setToDestroy = true;
        }
    }
    ...
    public void update(final float delta) {
        ...
        switch (getState()) {
            ...
            case DISABLE:
                ...
                disableTime -= delta;
                if (direction) {
                    setPosition(body.getPosition().x - getWidth() / 2 + 0.3f, body.getPosition().y - ConstEnemies.TROLL_HEAD_DIFF_SIZE);
                } else {
                    setPosition(body.getPosition().x - getWidth() / 2 - 0.3f, body.getPosition().y - ConstEnemies.TROLL_HEAD_DIFF_SIZE);
                }
                break;
            case ATTACK:
                setSize(ConstEnemies.TROLL_WIDTH + 0.8f, ConstEnemies.TROLL_HEIGHT + 2);
                if (attackAnimation.isAnimationFinished(attackTime) && plantAttack) {
                    ...
                    tree.setHp(ConstEnemies.TROLL_HIGH_DAMAGE);
                    attackTime = 0;
                }
                if (direction) {
                    body.setLinearVelocity(0, ConstEnemies.VELOCITY_Y);
    ...
    public int getEnemyDamage() {
        return ConstEnemies.TROLL_HIGH_DAMAGE;
    }

    @Override
    public int getScore() {
        return ConstEnemies.TROLL_HIGH_SCORE;
    }
