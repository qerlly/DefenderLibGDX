...
public class TrollDefault extends Enemy {

    public TrollDefault(final PlayScreen pScreen, final float pX, final float pY, final boolean pDirection) {
        ...
        if(pScreen.getType().contains(ConstGame.SURVIVE_MODE)){
            if(pScreen.getGameGui().getGenerates().getWaveIndex() % 5 == 0) {
                hp = (ConstEnemies.TROLL_DEFAULT_HP / 2) + ((pScreen.getGameGui().getGenerates().getWaveIndex() / 5) * 35);
            } else if(pScreen.getGameGui().getGenerates().getWaveIndex() > 4 ){
                hp = (ConstEnemies.TROLL_DEFAULT_HP / 2) + ((int)Math.floor(Math.abs(pScreen.getGameGui().getGenerates().getWaveIndex()/5)) * 35);
            } else {
                hp = ConstEnemies.TROLL_DEFAULT_HP / 2;
            }
        } else {
            hp = ConstEnemies.TROLL_DEFAULT_HP;
        }

        for (int i = 0; i < 6; i++) {
            frames.add(new TextureRegion(DefenderOfNature.getTrollsAtlas().findRegion("troll_default_run"), 0, i * 233, 344, 233));
        ...
        for (int j = 0; j < 4; j++) {
            for (int i = 0; i < 2; i++) {
                ...
                frames.add(new TextureRegion(DefenderOfNature.getTrollsAtlas().findRegion("troll_default_attack"), i * 394, j * 342, 394, 342));
    ...
    public void update(final float delta) {
        ...
        switch (getState()) {
            ...
            case DISABLE:
                ...
                break;
            case ATTACK:
                setSize(ConstEnemies.TROLL_WIDTH + 0.8f, ConstEnemies.TROLL_HEIGHT + 2.3f);
                if (attackAnimation.isAnimationFinished(attackTime) && plantAttack) {
                    ...
                    tree.setHp(ConstEnemies.TROLL_DEFAULT_DAMAGE);
                    attackTime = 0;
                }
                if (direction) {
                    body.setLinearVelocity(0, ConstEnemies.VELOCITY_Y);
    ...
    public int getEnemyDamage() {
        return ConstEnemies.TROLL_DEFAULT_DAMAGE;
    }

    @Override
    public int getScore() {
        return ConstEnemies.TROLL_DEFAULT_SCORE;
    }
