...
public class GoblinDefault extends Enemy {

    public GoblinDefault(final PlayScreen pScreen, final float pX, final float pY, final boolean pDirection) {
        ...
        if(pScreen.getType().contains(ConstGame.SURVIVE_MODE)){
            if(pScreen.getGameGui().getGenerates().getWaveIndex() % 5 == 0) {
                hp = (ConstEnemies.GOBLINS_DEFAULT_HP / 2) + ((pScreen.getGameGui().getGenerates().getWaveIndex() / 5) * 10);
            } else if(pScreen.getGameGui().getGenerates().getWaveIndex() > 4 ){
                hp = (ConstEnemies.GOBLINS_DEFAULT_HP / 2) + ((int)Math.floor(Math.abs(pScreen.getGameGui().getGenerates().getWaveIndex()/5)) * 10);
            } else {
                hp = ConstEnemies.GOBLINS_DEFAULT_HP / 2;
            }
        } else {
            hp = ConstEnemies.GOBLINS_DEFAULT_HP;
        }

        for (int i = 0; i < 5; i++) {
            frames.add(new TextureRegion(DefenderOfNature.getGoblinsAtlas().findRegion("goblin_default_run"), i * 128, 0, 128, 163));
            runAnimation = new Animation<TextureRegion>(ConstEnemies.GOBLINS_ANIM - 0.02f, frames, Animation.PlayMode.LOOP);
        }
        frames.clear();

        for (int i = 0; i < 5; i++) {
            frames.add(new TextureRegion(DefenderOfNature.getGoblinsAtlas().findRegion("goblin_default_attack"), i * 132, 0, 132, 155));
            attackAnimation = new Animation<TextureRegion>(ConstEnemies.GOBLINS_ANIM, frames, Animation.PlayMode.LOOP);
        }
        frames.clear();
        ...
        switch (getState()) {
            ...
            case ATTACK:
                if (attackAnimation.isAnimationFinished(attackTime) && plantAttack) {
                    ...
                    tree.setHp(ConstEnemies.GOBLINS_DEFAULT_DAMAGE);
                    attackTime = 0;
                }
                if (direction) {
                    body.setLinearVelocity(0, ConstEnemies.VELOCITY_Y);
    ...
    public int getEnemyDamage() {
        return ConstEnemies.GOBLINS_DEFAULT_DAMAGE;
    }

    @Override
    public int getScore() {
        return ConstEnemies.GOBLINS_DEFAULT_SCORE;
    }
