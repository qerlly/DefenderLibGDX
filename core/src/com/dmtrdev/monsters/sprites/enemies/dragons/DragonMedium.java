...
public class DragonMedium extends Enemy {

    public DragonMedium(final PlayScreen pScreen, final float pX, final float pY, final boolean pDirection) {
        ...
        if(pScreen.getType().contains(ConstGame.SURVIVE_MODE)){
            if(pScreen.getGameGui().getGenerates().getWaveIndex() % 5 == 0) {
                hp = (ConstEnemies.DRAGON_DEFAULT_HP / 2) + ((pScreen.getGameGui().getGenerates().getWaveIndex() / 5) * 30);
            } else if(pScreen.getGameGui().getGenerates().getWaveIndex() > 4 ){
                hp = (ConstEnemies.DRAGON_DEFAULT_HP / 2) + ((int)Math.floor(Math.abs(pScreen.getGameGui().getGenerates().getWaveIndex()/5)) * 30);
            } else {
                hp = ConstEnemies.DRAGON_DEFAULT_HP / 2;
            }
        } else {
            hp = ConstEnemies.DRAGON_DEFAULT_HP;
        }

        for (int i = 0; i < 5; i++) {
            frames.add(new TextureRegion(DefenderOfNature.getDragonsAtlas().findRegion("dragon_default_run"), i * 327, 0, 327, 242));
            runAnimation = new Animation<>(ConstEnemies.DRAGON_ANIM - 0.01f, frames, Animation.PlayMode.LOOP);
        }
        frames.clear();

        for (int i = 0; i < 5; i++) {
            frames.add(new TextureRegion(DefenderOfNature.getDragonsAtlas().findRegion("dragon_default_attack"), 0, i * 235, 509, 235));
        ...
        switch (getState()) {
            ...
            case ATTACK:
                ...
                if (attackAnimation.isAnimationFinished(attackTime) && plantAttack) {
                    ...
                    tree.setHp(ConstEnemies.DRAGON_DEFAULT_DAMAGE);
                    attackTime = 0;
                }
                if (direction) {
    ...
    public int getEnemyDamage() {
        return ConstEnemies.DRAGON_DEFAULT_DAMAGE;
    }

    @Override
    public int getScore() {
        return ConstEnemies.DRAGON_DEFAULT_SCORE;
    }
