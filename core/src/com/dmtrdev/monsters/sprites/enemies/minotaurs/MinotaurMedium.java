...
public class MinotaurMedium extends Enemy {

    private byte count;

    public MinotaurMedium(final PlayScreen pScreen, final float pX, final float pY, final boolean pDirection) {
        ...
        if(pScreen.getType().contains(ConstGame.SURVIVE_MODE)){
            if(pScreen.getGameGui().getGenerates().getWaveIndex() % 5 == 0) {
                hp = (ConstEnemies.MINOTAUR_MEDIUM_HP / 2) + ((pScreen.getGameGui().getGenerates().getWaveIndex() / 5) * 25);
            } else if(pScreen.getGameGui().getGenerates().getWaveIndex() > 4 ){
                hp = (ConstEnemies.MINOTAUR_MEDIUM_HP / 2) + ((int)Math.floor(Math.abs(pScreen.getGameGui().getGenerates().getWaveIndex()/5)) * 25);
            } else {
                hp = ConstEnemies.MINOTAUR_MEDIUM_HP / 2;
            }
        } else {
            hp = ConstEnemies.MINOTAUR_MEDIUM_HP;
        }

        for (int i = 0; i < 7; i++) {
            frames.add(new TextureRegion(DefenderOfNature.getMinotaursAtlas().findRegion("minotaur_medium_run"), 0, i * 274, 375, 274));
            runAnimation = new Animation<>(ConstEnemies.MINOTAUR_ANIM, frames, Animation.PlayMode.LOOP);
        }
        frames.clear();

        for (int i = 0; i < 7; i++) {
            frames.add(new TextureRegion(DefenderOfNature.getMinotaursAtlas().findRegion("minotaur_medium_attack"), 0, i * 274, 306, 274));
        ...
        switch (getState()) {
            ...
            case ATTACK:
                ...
                if (attackAnimation.isAnimationFinished(attackTime) && plantAttack) {
                    ...
                    tree.setHp(ConstEnemies.MINOTAUR_MEDIUM_DAMAGE);
                    attackTime = 0;
                    count++;
                    if (count >= 2) {
                        setDirection();
    ...
    public int getEnemyDamage() {
        return ConstEnemies.MINOTAUR_MEDIUM_DAMAGE;
    }

    @Override
    public int getScore() {
        return ConstEnemies.MINOTAUR_MEDIUM_SCORE;
    }
