...
public class MinotaurDefault extends Enemy {

    private byte count;

    public MinotaurDefault(final PlayScreen pScreen, final float pX, final float pY, final boolean pDirection) {
        ...
        if(pScreen.getType().contains(ConstGame.SURVIVE_MODE)){
            if(pScreen.getGameGui().getGenerates().getWaveIndex() % 5 == 0) {
                hp = (ConstEnemies.MINOTAUR_DEFAULT_HP / 2) + ((pScreen.getGameGui().getGenerates().getWaveIndex() / 5) * 25);
            } else if(pScreen.getGameGui().getGenerates().getWaveIndex() > 4 ){
                hp = (ConstEnemies.MINOTAUR_DEFAULT_HP / 2) + ((int)Math.floor(Math.abs(pScreen.getGameGui().getGenerates().getWaveIndex()/5)) * 25);
            } else {
                hp = ConstEnemies.MINOTAUR_DEFAULT_HP / 2;
            }
        } else {
            hp = ConstEnemies.MINOTAUR_DEFAULT_HP;
        }

        for (int i = 0; i < 7; i++) {
            frames.add(new TextureRegion(DefenderOfNature.getMinotaursAtlas().findRegion("minotaur_default_run"), 0, i * 269, 370, 269));
            runAnimation = new Animation<>(ConstEnemies.MINOTAUR_ANIM, frames, Animation.PlayMode.LOOP);
        }
        frames.clear();

        for (int i = 0; i < 7; i++) {
            frames.add(new TextureRegion(DefenderOfNature.getMinotaursAtlas().findRegion("minotaur_default_attack"), 0, i * 274, 317, 274));
    ...
    public void update(final float delta) {
        switch (getState()) {
            ...
            case DISABLE:
                ...
                disableTime -= delta;
                if (direction) {
                    setPosition(body.getPosition().x - getWidth() / 2 + 0.3f, body.getPosition().y - ConstEnemies.MINOTAUR_BODY_DIFF_SIZE);
                } else {
                    setPosition(body.getPosition().x - getWidth() / 2 - 0.3f, body.getPosition().y - ConstEnemies.MINOTAUR_BODY_DIFF_SIZE);
                }                break;
            case ATTACK:
                ...
                if (attackAnimation.isAnimationFinished(attackTime) && plantAttack) {
                    ...
                    tree.setHp(ConstEnemies.MINOTAUR_DEFAULT_DAMAGE);
                    attackTime = 0;
                    if (count >= 2) {
                        setDirection();
                        count = 0;
                ...
                break;
            default:
                setSize(ConstEnemies.MINOTAUR_SIZE, ConstEnemies.MINOTAUR_SIZE - 0.1f);
                if (direction) {
    ...
    public int getEnemyDamage() {
        return ConstEnemies.MINOTAUR_DEFAULT_DAMAGE;
    }

    @Override
    public int getScore() {
        return ConstEnemies.MINOTAUR_DEFAULT_SCORE;
    }
