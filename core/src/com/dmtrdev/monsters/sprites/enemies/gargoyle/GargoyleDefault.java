...
public class GargoyleDefault extends Enemy {

    public GargoyleDefault(final PlayScreen pScreen, final float pX, final float pY, final boolean pDirection) {
        ...
        if(pScreen.getType().contains(ConstGame.SURVIVE_MODE)){
            if(pScreen.getGameGui().getGenerates().getWaveIndex() % 5 == 0) {
                hp = (ConstEnemies.GARGOYLE_DEFAULT_HP / 2) + ((pScreen.getGameGui().getGenerates().getWaveIndex() / 5) * 15);
            } else if(pScreen.getGameGui().getGenerates().getWaveIndex() > 4 ){
                hp = (ConstEnemies.GARGOYLE_DEFAULT_HP / 2) + ((int)Math.floor(Math.abs(pScreen.getGameGui().getGenerates().getWaveIndex()/5)) * 15);
            } else {
                hp = ConstEnemies.GARGOYLE_DEFAULT_HP / 2;
            }
        } else {
            hp = ConstEnemies.GARGOYLE_DEFAULT_HP;
        }

        for (int i = 0; i < 5; i++) {
            frames.add(new TextureRegion(DefenderOfNature.getGargoyleAtlas().findRegion("gargoyle_default_run"), 0, i * 303, 359, 303));
            runAnimation = new Animation<>(ConstEnemies.GARGOYLE_ANIM, frames, Animation.PlayMode.LOOP);
        }
        frames.clear();

        for (int i = 0; i < 5; i++) {
            frames.add(new TextureRegion(DefenderOfNature.getGargoyleAtlas().findRegion("gargoyle_default_attack"), 0, i * 304, 350, 304));
        ...
        switch (getState()) {
            ...
            case ATTACK:
                ...
                if (attackAnimation.isAnimationFinished(attackTime) && plantAttack) {
                    ...
                    tree.setHp(ConstEnemies.GARGOYLE_DEFAULT_DAMAGE);
                    attackTime = 0;
                }
                if (direction) {
    ...
    public int getEnemyDamage() {
        return ConstEnemies.GARGOYLE_DEFAULT_DAMAGE;
    }

    @Override
    public int getScore() {
        return ConstEnemies.GARGOYLE_DEFAULT_SCORE;
    }
