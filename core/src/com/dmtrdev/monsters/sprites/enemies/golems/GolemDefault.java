...
public class GolemDefault extends Enemy {

    public GolemDefault(final PlayScreen pScreen, final float pX, final float pY, final boolean pDirection) {
        ...
        if(pScreen.getType().contains(ConstGame.SURVIVE_MODE)){
            if(pScreen.getGameGui().getGenerates().getWaveIndex() % 5 == 0) {
                hp = (ConstEnemies.GOLEM_DEFAULT_HP / 2) + ((pScreen.getGameGui().getGenerates().getWaveIndex() / 5) * 50);
            } else if(pScreen.getGameGui().getGenerates().getWaveIndex() > 4 ){
                hp = (ConstEnemies.GOLEM_DEFAULT_HP / 2) + ((int)Math.floor(Math.abs(pScreen.getGameGui().getGenerates().getWaveIndex()/5)) * 50);
            } else {
                hp = ConstEnemies.GOLEM_DEFAULT_HP / 2;
            }
        } else {
            hp = ConstEnemies.GOLEM_DEFAULT_HP;
        }

        for (int i = 0; i < 7; i++) {
            frames.add(new TextureRegion(DefenderOfNature.getGolemsAtlas().findRegion("golem_default_run"), 0, i * 244, 320, 244));
        ...
        for (int j = 0; j < 3; j++) {
            for (int i = 0; i < 3; i++) {
                ...
                frames.add(new TextureRegion(DefenderOfNature.getGolemsAtlas().findRegion("golem_default_attack"), i * 302, j * 297, 302, 297));
    ...
    public void update(final float delta) {
        ...
        switch (getState()) {
            ...
            case DISABLE:
                ...
                break;
            case ATTACK:
                setSize(ConstEnemies.GOLEM_SIZE - 1.2f, ConstEnemies.GOLEM_SIZE + 1.5f);
                if (attackAnimation.isAnimationFinished(attackTime) && plantAttack) {
                    ...
                    tree.setHp(ConstEnemies.GOLEM_DEFAULT_DAMAGE);
    ...
    public int getEnemyDamage() {
        return ConstEnemies.GOLEM_DEFAULT_DAMAGE;
    }

    @Override
    public int getScore() {
        return ConstEnemies.GOLEM_DEFAULT_SCORE;
    }
