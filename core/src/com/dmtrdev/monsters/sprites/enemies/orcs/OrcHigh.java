...
public class OrcHigh extends Enemy {

    public OrcHigh(final PlayScreen pScreen, final float pX, final float pY, final boolean pDirection) {
        ...
        if(pScreen.getType().contains(ConstGame.SURVIVE_MODE)){
            if(pScreen.getGameGui().getGenerates().getWaveIndex() % 5 == 0) {
                hp = (ConstEnemies.ORC_HIGH_HP / 2) + ((pScreen.getGameGui().getGenerates().getWaveIndex() / 5) * 10);
            } else if(pScreen.getGameGui().getGenerates().getWaveIndex() > 4 ){
                hp = (ConstEnemies.ORC_HIGH_HP / 2) + ((int)Math.floor(Math.abs(pScreen.getGameGui().getGenerates().getWaveIndex()/5)) * 10);
            } else {
                hp = ConstEnemies.ORC_HIGH_HP / 2;
            }
        } else {
            hp = ConstEnemies.ORC_HIGH_HP;
        }

        for (int j = 0; j < 3; j++) {
            for (int i = 0; i < 2; i++) {
                frames.add(new TextureRegion(DefenderOfNature.getOrcsAtlas().findRegion("orc_high_run"), i * 345, j * 359, 345, 359));
        ...
        for (int j = 0; j < 4; j++) {
            for (int i = 0; i < 2; i++) {
                ...
                frames.add(new TextureRegion(DefenderOfNature.getOrcsAtlas().findRegion("orc_high_attack"), i * 345, j * 359, 345, 359));
    ...
    public void damaged(final int pDamage, final String pData) {
        ...
        if (hp <= 0 || pData.equals(IronAxe.class.getSimpleName()) || pData.equals(PrimitiveAxe.class.getSimpleName())) {
            setToDestroy = true;
        }
    }
        ...
        switch (getState()) {
            ...
            case ATTACK:
                if (attackAnimation.isAnimationFinished(attackTime) && plantAttack) {
                    ...
                    tree.setHp(ConstEnemies.ORC_HIGH_DAMAGE);
                    attackTime = 0;
                }
                if (direction) {
                    body.setLinearVelocity(0, ConstEnemies.VELOCITY_Y);
    ...
    public int getEnemyDamage() {
        return ConstEnemies.ORC_HIGH_DAMAGE;
    }

    @Override
    public int getScore() {
        return ConstEnemies.ORC_HIGH_SCORE;
    }
