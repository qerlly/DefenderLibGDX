...
public class OrcDefault extends Enemy {

    public OrcDefault(final PlayScreen pScreen, final float pX, final float pY, final boolean pDirection) {
        ...
        if(pScreen.getType().contains(ConstGame.SURVIVE_MODE)){
            if(pScreen.getGameGui().getGenerates().getWaveIndex() % 5 == 0) {
                hp = (ConstEnemies.ORC_DEFAULT_HP / 2) + ((pScreen.getGameGui().getGenerates().getWaveIndex() / 5) * 10);
            } else if(pScreen.getGameGui().getGenerates().getWaveIndex() > 4 ){
                hp = (ConstEnemies.ORC_DEFAULT_HP / 2) + ((int)Math.floor(Math.abs(pScreen.getGameGui().getGenerates().getWaveIndex()/5)) * 10);
            } else {
                hp = ConstEnemies.ORC_DEFAULT_HP / 2;
            }
        } else {
            hp = ConstEnemies.ORC_DEFAULT_HP;
        }

        for (int i = 0; i < 6; i++) {
            frames.add(new TextureRegion(DefenderOfNature.getOrcsAtlas().findRegion("orc_default_run"), i * 324, 0, 324, 324));
        ...
        for (int j = 0; j < 4; j++) {
            for (int i = 0; i < 2; i++) {
                ...
                frames.add(new TextureRegion(DefenderOfNature.getOrcsAtlas().findRegion("orc_default_attack"), i * 345, j * 345, 345, 345));
    ...
    public void update(final float delta) {
        ...
        switch (getState()) {
            ...
            case DISABLE:
                ...
                break;
            case ATTACK:
                setSize(ConstEnemies.ORC_SIZE_WIDTH + 0.2f, ConstEnemies.ORC_SIZE_HEIGHT + 0.2f);
                if (attackAnimation.isAnimationFinished(attackTime) && plantAttack) {
                    ...
                    tree.setHp(ConstEnemies.ORC_DEFAULT_DAMAGE);
                    attackTime = 0;
                }
                if (direction) {
                    body.setLinearVelocity(0, ConstEnemies.VELOCITY_Y);
                    setPosition(body.getPosition().x - getWidth() / 2 + ConstEnemies.ORC_HEAD_DIFF_SIZE, body.getPosition().y - ConstEnemies.ORC_HEAD_DIFF_SIZE);
                } else {
                    body.setLinearVelocity(0, ConstEnemies.VELOCITY_Y);
                    setPosition(body.getPosition().x - getWidth() / 2 - ConstEnemies.ORC_HEAD_DIFF_SIZE, body.getPosition().y - ConstEnemies.ORC_HEAD_DIFF_SIZE);
                }
    ...
    public int getEnemyDamage() {
        return ConstEnemies.ORC_DEFAULT_DAMAGE;
    }

    @Override
    public int getScore() {
        return ConstEnemies.ORC_DEFAULT_SCORE;
    }
